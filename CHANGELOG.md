...
Версионирование следующее: до `1.0.0` вместо major используется minor:
`0.x.0` — breaking change, `0.x.y` — обратно совместимый фикс/фича.

## Unreleased

### Добавлено

- Mailpit в `compose.yaml` для проверки писем при локальной
  разработке.
- Домохозяйства: модель `Household`, миграция таблицы
  `households`, привязка рецептов и дней меню к домохозяйству
  (`household_id`).
- Фабрика и сидер домохозяйства (`HouseholdFactory`,
  `HouseholdSeeder`); сидер пользователя вынесен в отдельный
  `UserSeeder`.
